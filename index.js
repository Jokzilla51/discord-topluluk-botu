...
// 6.2. Üyenin Klan Tagını (VYRN / Vyron) Taşıyıp Taşımadığını Kontrol Etme
function memberHasTag(member, tagText = 'VYRN') {
  ...
  return username.includes(searchTag) || displayName.includes(searchTag) || globalName.includes(searchTag) ||
         username.includes('vyrn') || displayName.includes('vyrn') || globalName.includes('vyrn') ||
         username.includes('vyron') || displayName.includes('vyron') || globalName.includes('vyron');
}
...
const commands = [
  ...
  new SlashCommandBuilder()
    .setName('tag-tara')
    .setDescription('Sunucudaki tüm üyeleri tarar, tagı olanları kaydeder ve tagı bırakanları raporlar.')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
];
    ...
    // DURUM 2: Tag Bırakıldı veya Başka Bir Taga Geçildi
      ...
      if (chTagLog) {
        const leaveEmbed = new EmbedBuilder()
          ...
          .setTitle('🔴 BİR ÜYE KLAN TAGINI BIRAKTI / DEĞİŞTİRDİ!')
          .setThumbnail(newMember.user.displayAvatarURL({ dynamic: true, size: 256 }))
          .addFields(
            { name: '👤 Kullanıcı', value: `${newMember} (\`${newMember.user.tag}\`)`, inline: true },
            { name: '🆔 Discord ID', value: `\`${newMember.id}\``, inline: true },
            { name: '🏷️ Güncel İsim', value: `\`${newMember.displayName}\``, inline: true },
            { name: '⏰ İşlem Tarihi', value: `<t:${Math.floor(Date.now() / 1000)}:F>`, inline: false },
            { name: '⚠️ Tespit', value: `Tag isminden silindi veya başka bir klan tagına geçildi.`, inline: false }
          )
          .setFooter({ text: `${FOOTER_TEXT} • Güvenlik & Tag Takibi` })
          .setTimestamp();
  ...
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (commandName === 'tag-tara') {
        ...
        const clanRole = guild.roles.cache.find(r => r.name.toLowerCase().includes('klan üye') && !r.name.toLowerCase().includes('has'));
        ...
        const scanEmbed = new EmbedBuilder()
          ...
          .setTitle('🏷️ KLAN TAGI TARAMA & SENKRONİZASYON RAPORU')
          .setDescription(
            `Sunucudaki tüm üyeler tarandı ve tag durumları hafızaya senkronize edildi.\n\n` +
            `📊 **Toplam Tag Alan Üye:** \`${tagCount} Kişi\`\n` +
            `🏷️ **Aktif Tag Metni:** \`${data.tagText || 'VYRN'} / Vyron\`\n` +
            `📢 **Tag Log Kanalı:** ${chTagLog ? chTagLog : '`#tag-log`'}\n\n` +
            `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
            `⚠️ **Tagı Olmayan Klan Üyeleri (${nonTagClanMembers.length} Kişi):**\n` +
            (nonTagClanMembers.length > 0
              ? nonTagClanMembers.slice(0, 15).map(m => `• ${m} (\`${m.user.tag}\`)`).join('\n') + (nonTagClanMembers.length > 15 ? `\n*...ve ${nonTagClanMembers.length - 15} kişi daha.*` : '')
              : '✅ *Tüm klan üyelerinde klan tagı eksiksiz bulunmaktadır!*')
          )
          .setFooter({ text: FOOTER_TEXT })
          .setTimestamp();
